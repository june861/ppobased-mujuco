...
class BaseRunner(ABC):
    ...
    def setup_logging(self):
        ...
        self.run_name = f"{self.all_args.exp_name}_{int(time.time())}_{os.getppid()}"
        self.all_args.logger.info(f"wandb project is {self.all_args.wandb_project_name}, run name is {self.run_name}, group name is {self.all_args.exp_name}!")
        if self.all_args.track:
            wandb.init(
                project=self.all_args.wandb_project_name,
                group=self.all_args.exp_name,

...
class AtariBuffer(BaseBuffer):
    def __init__(self, args):
        super().__init__(args)
        self.obs = torch.zeros((args.num_steps, args.num_envs, args.observation_shape)).to(args.device)
        ...
        self.total_logits = torch.zeros((args.num_steps, args.num_envs, args.discrete_action_n), dtype=torch.float32).to(args.device)
